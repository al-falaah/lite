...
serve(async (req) => {
  ...
  try {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      // Referral: if this checkout used a promotion code, log the redemption.
      // We re-retrieve the session with expansion because the event payload
      // doesn't include promotion_code by default.
      try {
        const fullSession = await stripe.checkout.sessions.retrieve(session.id, {
          expand: ['total_details.breakdown.discounts', 'total_details.breakdown.discounts.discount.promotion_code'],
        })
        const discount = fullSession.total_details?.breakdown?.discounts?.[0]
        const promoCode = discount?.discount?.promotion_code
        const promoCodeString = typeof promoCode === 'string' ? null : promoCode?.code
        if (promoCodeString) {
          // Look up referrer by their referral_code
          const { data: referrer } = await supabaseClient
            .from('students')
            .select('id')
            .eq('referral_code', promoCodeString)
            .maybeSingle()
          if (referrer && referrer.id !== studentId) {
            const { error: refErr } = await supabaseClient
              ...
              .insert({
                ...
                referral_code: promoCodeString,
                program: program,
                enrollment_id: enrollment,
                stripe_session_id: session.id,
                discount_amount: discount.amount ? discount.amount / 100 : null,
              })
            if (refErr && refErr.code !== '23505') {
              console.error('Referral log error:', refErr)
            } else {
              console.log('Referral logged:', promoCodeString, '->', studentId)
            }
          }
        }
      } catch (refLookupErr) {
        console.error('Referral lookup failed (non-fatal):', refLookupErr)
      }

      // Referral: issue a Stripe promotion code for new students so they can
      // share their code. The referral_code was auto-generated by the DB trigger.
      if (isNewStudent && student.referral_code) {
        try {
          ...
          console.log('Promotion code created:', student.referral_code)
        } catch (promoErr: any) {
          // Already exists = fine (e.g. replay or manual creation)
          if (promoErr?.code !== 'resource_already_exists') {
            console.error('Promotion code create failed (non-fatal):', promoErr?.message || promoErr)
          }
        }
      }
